...
export default function Home() {
  ...
  const quizHref = selectedWorld && (layoutMode === "web" ? `/quiz/${selectedWorld.slug}` : `/quiz/${selectedWorld.slug}?layout=${layoutMode}`);
  ...
  return (
    <main className="home-shell">
      ...
      <SiteHeader />
      <section className="hero"><span className="hero-badge">한글 · 그리기 놀이</span><h1>어떤 친구와<br /><em>오늘 놀아볼까?</em></h1><p>좋아하는 친구를 골라 퀴즈를 풀거나 그림을 그려봐요!</p>
      ...
      <section className="world-grid" aria-label="캐릭터 세계 선택">
        {quizWorlds.map((world, index) => (
          <button className="world-card" type="button" key={world.slug} style={{ "--card": world.color, "--soft": world.softColor, "--delay": `${index * 90}ms` } as React.CSSProperties} onClick={(event) => { triggerRef.current = event.currentTarget; setSelectedWorld(world); }} aria-haspopup="dialog">
      ...
      {selectedWorld && (
        <div className="activity-backdrop" onMouseDown={(event) => { if (event.target === event.currentTarget) setSelectedWorld(null); }}>
          <section className="activity-modal" role="dialog" aria-modal="true" aria-labelledby="activity-title" style={{ "--theme": selectedWorld.color, "--soft": selectedWorld.softColor } as React.CSSProperties}>
            <button ref={closeButtonRef} className="activity-close" type="button" onClick={() => setSelectedWorld(null)} aria-label="닫기">×</button>
            <div className="activity-character"><Image src={selectedWorld.cover} alt="" width={112} height={112} unoptimized /></div>
            ...
            <div className="activity-options">
              <Link className="activity-option quiz-option" href={quizHref || "#"}>
                <span aria-hidden="true">✎</span><div><strong>이름 퀴즈</strong><small>이름을 따라 쓰고 맞혀봐요</small></div><b aria-hidden="true">›</b>
              </Link>
              <Link className="activity-option draw-option" href={`/draw/${selectedWorld.slug}`}>
                <span aria-hidden="true">🎨</span><div><strong>캐릭터 그리기</strong><small>도감 가이드를 따라 그려봐요</small></div><b aria-hidden="true">›</b>
              </Link>
            </div>
          </section>
        </div>
