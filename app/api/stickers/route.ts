...
export async function GET(request: Request) {
  ...
  void _legacyAdmin;
  const storedRules =
    "rules" in stickers && stickers.rules && typeof stickers.rules === "object"
      ? (stickers.rules as Partial<Record<RewardKind, number>>)
      : {};
  return Response.json({
    state: {
      ...
      ...stickers,
      configVersion: gameRewardConfig.version,
      rules: { ...defaultRules, ...storedRules },
...
export async function POST(request: Request) {
  try {
    ...
    console.error("Sticker request failed", error);
    return Response.json(
      { error: code },
      { status: code.startsWith("invalid_") ? 400 : 500 },
    );
  }
}
