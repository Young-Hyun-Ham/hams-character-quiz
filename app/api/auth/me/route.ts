...
export async function GET(request: Request) {
  ...
  try {
    ...
    const membership = user?.serviceMemberships?.find((item) => item.clientId === getSsoClientId());
    const headerUser: HeaderUser | null = user ? {
      ...
      membership: membership ? { serviceName: membership.serviceName, plan: membership.plan } : null,
    } : null;
