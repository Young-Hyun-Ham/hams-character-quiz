...
function parseInquiry(body: unknown) {
  if (!body || typeof body !== "object") throw new ValidationError("문의 내용을 확인해 주세요.");
  ...
  if (title.length > 80) throw new ValidationError("제목은 80자 이하로 입력해 주세요.");
  if (!content) throw new ValidationError("내용을 입력해 주세요.");
  if (content.length > 200) throw new ValidationError("내용은 200자 이하로 입력해 주세요.");
  if (tags.length > 5) throw new ValidationError("태그는 최대 5개까지 입력할 수 있습니다.");
  if (tags.some((tag) => tag.length > 20)) throw new ValidationError("태그 하나는 20자 이하로 입력해 주세요.");
...
export async function GET() {
  try {
    const snapshot = await getAdminFirestore().collection("inquiries").orderBy("createdAt", "desc").get();
...
export async function POST(request: Request) {
  try {
    ...
    const reference = await getAdminFirestore().collection("inquiries").add({
      ...input,
      commentCount: 0,
    ...
    return Response.json({ error: status === 400 ? message : "문의를 등록하지 못했습니다." }, { status });
  }
}
